...
internal sealed class ILEmitResolverVisitor : CallSiteVisitor<ILEmitResolverBuilderContext, object>
{
    ...
    public ILEmitResolverVisitor(ServiceProvider serviceProvider)
    {
    ...
    public Func<ServiceProviderEngineScope, object> Build(CallSiteService callSite) => BuildType(callSite).Lambda;
    private GeneratedMethod BuildType(CallSiteService callSite)
    {
        // Only scope methods are cached
        return callSite.Cache.Location == CallSiteResultCacheLocation.Scope ? 
            _scopeResolverCache.GetOrAdd(callSite.Cache.Key, _buildTypeDelegate, callSite) : 
            BuildTypeNoCache(callSite);
    }
    ...
    private void AddCacheKey(ILEmitResolverBuilderContext argument, CallSiteServiceCacheKey key)
    {
