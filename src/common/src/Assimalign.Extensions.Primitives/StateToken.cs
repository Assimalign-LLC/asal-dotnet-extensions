...
public static class StateToken
{
    ...
    /// <returns></returns>
    public static IDisposable OnChange<TState>(Func<IStateToken> changeTokenProducer, Action<TState> changeTokenConsumer, TState state)
    {
        if (changeTokenProducer == null)
        {
            throw new ArgumentNullException(nameof(changeTokenProducer));
    ...
    {
        private readonly Func<IStateToken> _changeTokenProducer;
        ...
        public ChangeTokenRegistration(Func<IStateToken> changeTokenProducer, Action<TState> changeTokenConsumer, TState state)
        {
            _changeTokenProducer = changeTokenProducer;
            _changeTokenConsumer = changeTokenConsumer;
            _state = state;

            IStateToken token = changeTokenProducer();
        ...
        private void OnChangeTokenFired()
        {
            ...
            IStateToken token = _changeTokenProducer();
        ...
        private void RegisterChangeTokenCallback(IStateToken token)
        {
            if (token is null)
            {
                return;
