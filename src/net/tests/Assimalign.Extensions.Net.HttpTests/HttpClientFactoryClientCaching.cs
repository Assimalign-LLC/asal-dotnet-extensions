...
namespace Assimalign.Extensions.Net.HttpTests;

using Assimalign.Extensions.Net.Http;
public class HttpClientFactoryClientCaching
{
  

    [Fact]
    public void HttpClientFactoryShouldNotCacheClientTest()
    { 
        // Arrange
        var count = 0;
        var factory = HttpClientFactory.CreateHttpClientFactory(configure =>
        {
            configure.AddHttpClientAction(client =>
            {
                count++;
            });
        });
    ...
    [Fact]
    public void HttpClientFactoryShouldCacheClientTest()
    {
        // Arrange
        var count = 0;
        var factory = HttpClientFactory.CreateHttpClientFactory(configure =>
        {
            configure.AddHttpHandlerBuilderAction(builder =>
            {
                count++;
                builder.AdditionalHandlers.Add(new Temp());
            });
        });
...
public class Temp : DelegatingHandler
{
    protected override void Dispose(bool disposing)
    {
