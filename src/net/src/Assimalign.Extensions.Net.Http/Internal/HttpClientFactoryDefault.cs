...
namespace Assimalign.Extensions.Net.Http
{
    ...
    internal class HttpClientFactoryDefault : IHttpClientFactory, IHttpMessageHandlerFactory
    {
        private static readonly TimerCallback _cleanupCallback = (s) => ((HttpClientFactoryDefault)s).CleanupTimer_Tick();
        ...
        //
        // internal for tests
        internal readonly ConcurrentQueue<HttpMessageHandlerExpiredTrackingEntry> _expiredHandlers;
        private readonly TimerCallback _expiryCallback;

        public HttpClientFactoryDefault(
            ...
            IEnumerable<IHttpMessageHandlerBuilderFilter> filters)
        {
            ...
            _logger = loggerFactory.CreateLogger<HttpClientFactoryDefault>();
            ...
            _expiredHandlers = new ConcurrentQueue<HttpMessageHandlerExpiredTrackingEntry>();
        ...
        internal void CleanupTimer_Tick()
        {
            ...
            try
            {
                ...
                for (int i = 0; i < initialCount; i++)
                {
                    ...
                    _expiredHandlers.TryDequeue(out HttpMessageHandlerExpiredTrackingEntry entry);
